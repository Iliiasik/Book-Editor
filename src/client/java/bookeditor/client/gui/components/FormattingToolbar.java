...
public class FormattingToolbar {
    ...
    private TextFieldWidget sizeField; private ButtonWidget applySizeBtn;
    ...
    public void build() {
        ...
        undoBtn = addBtn("↶", b -> { if (editor.undo()) onDirty.run(); }, cx); cx += 18 + gap;
        redoBtn = addBtn("↷", b -> { if (editor.redo()) onDirty.run(); }, cx); cx += 18 + gap;

        boldBtn = addBtn("B", b -> { editor.setBold(!editor.isBold()); refreshFormatButtons(); onDirty.run(); }, cx); cx += 18 + gap;
        italicBtn = addBtn("I", b -> { editor.setItalic(!editor.isItalic()); refreshFormatButtons(); onDirty.run(); }, cx); cx += 18 + gap;
        underlineBtn = addBtn("U", b -> { editor.setUnderline(!editor.isUnderline()); refreshFormatButtons(); onDirty.run(); }, cx); cx += 18 + gap;

        sizeField = new TextFieldWidget(host.getTextRenderer(), cx, y, 40, btnH, Text.translatable("screen.bookeditor.size"));
        sizeField.setText("1.0"); host.addDrawable(sizeField); cx += 40 + gap;
        ...
        alignLeftBtn   = addBtn("L", b -> { editor.setAlignment(BookData.ALIGN_LEFT); onDirty.run(); }, cx); cx += 18 + gap;
        alignCenterBtn = addBtn("C", b -> { editor.setAlignment(BookData.ALIGN_CENTER); onDirty.run(); }, cx); cx += 18 + gap;
        alignRightBtn  = addBtn("R", b -> { editor.setAlignment(BookData.ALIGN_RIGHT); onDirty.run(); }, cx); cx += 18 + gap;

        textColorBtn = new ColorPickerButton(cx, y, argb -> {
            editor.setColor(argb); editor.setBrushColor(argb); onDirty.run();
        }, 0xFF202020);
    ...
    public void setFontSizeField(float s) {
        if (sizeField != null) sizeField.setText(Float.toString(s));
    }
}